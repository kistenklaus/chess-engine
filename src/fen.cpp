...
ChessPosition fen::parsePosition(const std::string &FEN) {
  ...
  Board board =
      Board(b_pawns, w_pawns, b_bishops, w_bishops, b_knights, w_knights,
            b_rooks, w_rooks, b_queen, w_queen, b_king, w_king);

  GameState state =
      GameState(white_turn, has_en_passant, white_long_castle,
                white_short_castle, black_long_castle, black_short_castle);

  return ChessPosition(board, state, epTarget);
}

[[deprecated]] Board fen::parse(const std::string &FEN) {
  std::string split[6];
  int index = 0;
  for (const char c : FEN) {
    if (c == ' ') {
