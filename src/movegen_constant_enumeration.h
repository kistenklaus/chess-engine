...
inline void _enumeration(const Board &board, MoveReceiver &receiver,
  ...
  // Pawnmoves
  {
    ...
    bitmap_t leftAttackingPawns =
        pawnsD12 & shift::PawnUndoAttackLeft<turn>(board.OccupiedBy<!turn>() &
                                            shift::PawnsNotRight() & checkmask);
    bitmap_t rightAttackingPawns =
        pawnsD12 & shift::PawnUndoAttackRight<turn>(board.OccupiedBy<!turn>() &
                                             shift::PawnsNotLeft() & checkmask);

    bitmap_t pushablePawns = pawnsHV & shift::PawnBackward<turn>(board
                                                                      .NotOccupied());
    ...
    if ((leftAttackingPawns | rightAttackingPawns | pushablePawns) &
      ...
      bitmap_t promotePush = pushablePawns &
                             shift::RelativeSecondLastRank<turn>();
      ...
      bitmap_t noPromoteRight =
          rightAttackingPawns & shift::RelativeSecondLastRank<turn>();
      bitmap_t noPromotePush = pushablePawns &
                               shift::RelativeSecondLastRank<turn>();
...
never_inline void recursive_call(const Board &board, MoveReceiver &receiver) {
  ...
  if (checkmask != 0) {
    constant_enumeration::_enumeration<state, MoveReceiver, depth>(
...
template<GameState state, int depth, typename MoveReceiver>
void entry_call(const Board& board, bitmap_t epTarget, MoveReceiver& receiver){
  movegen::Movestack::init<state, depth>(board, epTarget);
  recursive_call<state, depth>(board, receiver);
}

template<typename MoveReceiver>
void runtime_entry_call(const Board& board, const GameState& state,
                        bitmap_t epTarget, MoveReceiver& receiver){
  if(state.turn()){
    if(state.hasEnPassant()){
      if(state.whiteHasLongCastle()){
        if(state.whiteHasShortCastle()){
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, true, true, true, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, true, true, true, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, true, true, true, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, true, true, true, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }else{
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, true, true, false, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, true, true, false, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, true, true, false, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, true, true, false, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }
        ...
        if(state.whiteHasShortCastle()){
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, true, false, true, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, true, false, true, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, true, false, true, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, true, false, true, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }else{
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, true, false, false, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, true, false, false, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, true, false, false, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, true, false, false, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }
      ...
      if(state.whiteHasLongCastle()){
        if(state.whiteHasShortCastle()){
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, false, true, true, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, false, true, true, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, false, true, true, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, false, true, true, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }else{
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, false, true, false, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, false, true, false, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, false, true, false, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, false, true, false, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }
        ...
        if(state.whiteHasShortCastle()){
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, false, false, true, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, false, false, true, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, false, false, true, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, false, false, true, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }else{
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, false, false, false, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, false, false, false, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(true, false, false, false, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(true, false, false, false, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }
    ...
    if(state.hasEnPassant()){
      if(state.whiteHasLongCastle()){
        if(state.whiteHasShortCastle()){
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, true, true, true, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, true, true, true, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, true, true, true, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, true, true, true, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }else{
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, true, true, false, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, true, true, false, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, true, true, false, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, true, true, false, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }
        ...
        if(state.whiteHasShortCastle()){
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, true, false, true, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, true, false, true, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, true, false, true, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, true, false, true, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }else{
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, true, false, false, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, true, false, false, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, true, false, false, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, true, false, false, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }
      ...
      if(state.whiteHasLongCastle()){
        if(state.whiteHasShortCastle()){
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, false, true, true, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, false, true, true, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, false, true, true, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, false, true, true, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }else{
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, false, true, false, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, false, true, false, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, false, true, false, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, false, true, false, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }
        ...
        if(state.whiteHasShortCastle()){
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, false, false, true, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, false, false, true, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, false, false, true, false, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, false, false, true, false, false),
                  0>(board, epTarget, receiver);
            }
          }
        }else{
          if(state.blackHasLongCastle()){
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, false, false, false, true, true),
                  0>(board, epTarget, receiver);
            }else{
              entry_call<GameState(false, false, false, false, true, false),
                  0>(board, epTarget, receiver);
            }
          }else{
            if(state.blackHasShortCastle()){
              entry_call<GameState(false, false, false, false, false, true),
                  0>(board, epTarget, receiver);
            }else{
